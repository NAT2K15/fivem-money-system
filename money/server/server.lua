...
AddEventHandler('NAT2K15:CHECKSQL', function(playerid, steam, discord, first_name, last_name, twt, dept, dob, gender, data) 
    local src = source
    ...
    MySQL.Async.fetchAll("SELECT * FROM money WHERE steam = @steam AND first = @first AND last = @last AND dept = @dept", {["@steam"] = steam, ["@first"] = first_name, ["@last"] = last_name, ["@dept"] = dept}, function(data) 
        if(data[1] == nil) then
            ...
            accounts[src] = {steam = steam, discord = discord, amount = 0, bank = config.starting_money, cycle = config.deptPay[dept], dept = dept, first = first_name, last = last_name}
            TriggerClientEvent('NAT2K15:UPDATECLIENTMONEY', src, accounts[src])
        else 
            accounts[src] = {steam = steam, discord = discord, amount = data[1].amount, bank = data[1].bank, cycle = config.deptPay[dept], dept = dept, first = first_name, last = last_name}
            TriggerClientEvent('NAT2K15:UPDATECLIENTMONEY', src, accounts[src])
        end
    end)
end)
...
AddEventHandler('playerDropped', function(reason) 
    ...
    if(accounts[src]) then 
        MySQL.Async.fetchAll("SELECT * FROM money WHERE steam = @steam AND first = @first AND last = @last AND dept = @dept", {["@steam"] = accounts[src].steam, ["@first"] =  accounts[src].first, ["@last"] =  accounts[src].last, ["@dept"] =  accounts[src].dept}, function(data) 
            if(data[1] ~= nil) then
                MySQL.Async.execute("UPDATE money SET amount = @amount, bank = @bank WHERE steam = @steam AND first = @first AND last = @last AND dept = @dept", {["@steam"] = accounts[src].steam, ["@amount"] = accounts[src].amount, ["@bank"] = accounts[src].bank, ["@first"] =  accounts[src].first, ["@last"] =  accounts[src].last, ["@dept"] =  accounts[src].dept})
        ...
        for _, player in ipairs(GetPlayers()) do
            ...
            if(accounts[player]) then
                if(accounts[player].dept) then
                    accounts[player].bank =  config.deptPay[accounts[player].dept] + accounts[player].bank
