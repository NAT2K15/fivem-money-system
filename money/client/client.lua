...
RegisterNetEvent('NAT2K15:UPDATECLIENTMONEY', function(coolarray)
...
RegisterNetEvent('NAT2K15:UPDATEPAYCHECK', function(moneyarray) 
    stuff = moneyarray
    local id = GetPlayerServerId(PlayerId())
    if(stuff[id]) then
...
RegisterNetEvent('NAT2K15:UPDATEPAY', function(id, money) 
    stuff = money
    SendNUIMessage({action = "update_money", amount = stuff[id].amount, bank = stuff[id].bank})
end)

RegisterNetEvent('NAT2K15:BANKNOTIFY', function(msg) 
    BeginTextCommandThefeedPost("STRING")
